...
static struct xen_domain_iomem domd_iomems[] = {
    ...
    {.first_mfn = 0x47fc7, .nr_mfns = 0x2, .first_gfn=0x37fc7}, // iccom shared memory(CR52)
    {.first_mfn = 0xe6860, .nr_mfns = 0x1}, /* UFS HCD */
    {.first_mfn = 0xe6078, .nr_mfns = 0x1},
    // vPCI: These are disabled in Dom0 Linux version.
    // But, Dom0 Zephyr requres these settings to avoid kernel panic.
    {.first_mfn = 0xe65d0, .nr_mfns = 0x3},
    {.first_mfn = 0xfe000, .nr_mfns = 0x10},
};
...
struct xen_domain_cfg domd_cfg = {
    ...
    .image_info        = NULL,
    .cmdline           = "root=/dev/mmcblk0p2 rw rootwait console=hvc0 clk_ignore_unused",
